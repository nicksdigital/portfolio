...
# Run migrations (only if there are no errors)
echo "Running database migrations..."
npm run db:migrate

# Only run seed if migrations succeeded
if [ $? -eq 0 ]; then
  echo "Seeding database with initial data..."
  npm run db:seed
else
  echo "Migrations failed, skipping database seeding."
  exit 1
fi
